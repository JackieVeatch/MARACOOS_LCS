...
function varargout = eig_cgStrain_edited(derivative,domain,resolution,timespan,varargin)
...
switch method
    case 'finiteDifference'
        ...
        if nargout == 2 || eigenvalueFromMainGrid == false
            ...
            % some NaN values in finalPositionMainGrid if the boundary conditions aren't handled well, fill with a linear interpolation
%             finalPositionAuxGrid = fillmissing(finalPositionAuxGrid, 'linear', 2); % Linear interpolation along columns
%             finalPositionAuxGrid = fillmissing(finalPositionAuxGrid, 'linear', 1); % Linear interpolation along rows
            
            cgStrainAuxGrid = compute_cgStrain(finalPositionAuxGrid,auxiliaryPosition,resolution);
            
            [cgStrainV,cgStrainD, abort] = arrayfun(@(x11,x12,x22)eig_array(x11,x12,x22,customEigMethod),cgStrainAuxGrid(:,1),cgStrainAuxGrid(:,2),cgStrainAuxGrid(:,3),'UniformOutput',false);
            
            abort = cellfun(@str2double, abort);
            abort = sum(abort);
    ...
    case 'equationOfVariation'
        ...
        [cgStrainV,cgStrainD] = arrayfun(@(a1,a2,a4)eig_array(a1,a2,a4,customEigMethod),squeeze(cgStrain(1,1,:)),squeeze(cgStrain(1,2,:)),squeeze(cgStrain(2,2,:)),'UniformOutput',false);
        cgStrainV = cell2mat(cgStrainV);
        cgStrainD = cell2mat(cgStrainD);
end
...
function [v,d, abort] = eig_array(x11,x12,x22,customMethod)

abort = false;

if isnan(x11) || isinf(x11) || isnan(x12) || isinf(x12) || isnan(x22) || isinf(x22)
    abort = true;
    v = [];
    d = [];
%     disp('WARNING: NaN or Inf detected');
