...
for d = 1:numSpringDays
    % index in time for one day
    indDay = find(MARACOOS.dt >= uniqueDates_S(d) & MARACOOS.dt < uniqueDates_S(d)+days(1));
    u = MARACOOS.u(:,:,indDay);
    v = MARACOOS.v(:,:,indDay);
...
%%
    ...
    for i=uniqueDates_S(d):hours(1):uniqueDates_S(d)+hours(23)
...
save('MARACOOS_filled_2013_spring' , 'MARACOOS_filled');
